...
public class LeastLoaded {
	...
	public ArrayList<String> search(Graph graph, String fromName, String toName){
		...
		Comparator<State> comparator = new ConnectionComparator();
		...
		while(!q.isEmpty() && !isVisited(toName)){
			...
			for (Edge currentEdge: toAddEdge){
				...
				double currentLoad = currentEdge.getCurrentLoad() + currentState.getCurrentLoad();
				State newState = new State(stateName, currentState, 0, currentLoad);
